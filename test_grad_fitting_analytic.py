...
relu_pe_model = ReLU_PE_Model([2,256,256,256,256,1], L=10).cuda()
...
for iter_idx in range(10000):
    relu_recon, relu_grad = relu_model(input_grid, get_gradient=True)
    relu_pe_recon, relu_pe_grad = relu_pe_model(input_grid, get_gradient=True)
    siren_recon, siren_grad = siren_model(input_grid, get_gradient=True)
